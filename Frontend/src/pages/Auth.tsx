  ...
  const handleGitHubSignIn = async () => {
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        ...
        toast.error("An account already exists with this email. Please try signing in with Google instead.");
      } else {
        ...
        <Card className="card-elevated p-8">
          <div className="space-y-6">
            {/* Google Sign In */}
              <Button 
                variant="outline" 
                className="w-full bg-background/50 hover:bg-secondary border-2 border-border/50 hover:border-primary/20"
                size="lg"
              onClick={handleGoogleSignIn}
              disabled={isGoogleLoading || isGitHubLoading || authLoading}
              >
              {isGoogleLoading ? (
